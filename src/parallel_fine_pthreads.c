...
void *thread_score(void* data){
	pthread_mutex_t mutex1 = PTHREAD_MUTEX_INITIALIZER;
	int score;
    int gap1;
    int gap2;
    struct Score_var *sc = (struct Score_var *)data;
    int s=sc->begin;int e=sc->end;
    int tmp=sc->tmp, temp=sc->temp,a=sc->al;
	for (int j = s; j <e; j++){
		if( j!=0 && tmp+a+temp-j-1!=0 ){
		if(sc->sq0[j-1]==sc->sq1[tmp+a+temp-j-1-1]){
			 score=sc->dp[j-1][tmp+a+temp-j-1-1]+match;
			 
			}
		else{
			
			 score=sc->dp[j-1][tmp+a+temp-j-1-1]+mismatch;
			}
			gap1=sc->dp[j-1][tmp+a+temp-j-1]+gap;
            gap2=sc->dp[j][tmp+a+temp-j-1-1]+gap;
            sc->dp[j][tmp+a+temp-j-1]=maxnumber(score,gap1,gap2);
            // 
            if(sc->dp[j][tmp+a+temp-j-1]>maxi) 
            	{	//fprintf(stderr, "TRYING TO FIND THAT MAX....\n" );
            	pthread_mutex_lock( &mutex1 );
              maxi=sc->dp[j][tmp+a+temp-j-1];
              pthread_mutex_unlock( &mutex1 );}

        }


	//fprintf(stderr, "tmp=%d j=%d a=%d k=%d",tmp,j,a,tmp+a+temp-j-1 );
//	fprintf(stderr, " VALUE!!!!!!!!!!! %d ", sc->dp[j][tmp+a+temp-j-1] );
...
for(int q=0; q < w-2 ; q+=2 )
{
	...
	int end1=length[q+2];
	//fprintf(stderr, "Start0:%d End0:%d Start1:%d End1:%d \n",start0,end0,start1,end1 );
	...
	//fprintf(stderr, "%c",source[i] );
			...
			int min,tmp=0,temp=0,l,tmp_threads=0;
			a=0;
			
			if(r>c)min=c;else min=r;
			struct Score_var st[threads];
			for (int i = 0; i < r+c-1; i++) {
				...
				if(threads>tmp)tmp_threads=threads-tmp;
				else tmp_threads=threads;
				pthread_t pthreads[tmp_threads];
				
				pthread_barrier_t barrier; 
				pthread_barrier_t barrier1; 
				

				int chunk=tmp/tmp_threads;
				
				
			

				
				pthread_barrier_init(&barrier, NULL, tmp_threads);
				//pthread_barrier_init(&barrier1, NULL, 1);
				for(int e=0;e<(int)tmp_threads;e++){
					st[e].sq0=seq0;
					st[e].sq1=seq1;
					st[e].al=a;
					st[e].tmp=tmp;
					...
					st[e].bar=barrier;
					st[e].dp=dp;
					st[e].begin=(e*chunk)+a;
					st[e].end=((e+1)*chunk)+a;
					if(e==tmp_threads-1)st[e].end=tmp+a;
					pthread_create(&pthreads[e],SCHED_OTHER,thread_score,(void*)&st[e]);
				...
				for(int e=0;e<(int)tmp_threads;e++){
					(void)pthread_join(pthreads[e],NULL);
				}
